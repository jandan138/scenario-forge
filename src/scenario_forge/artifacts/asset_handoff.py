    ...
    *, archive_id: str, packages: list[Path], output_dir: Path
    ...
    for source in packages:
        ...
        if (
            ...
            raise ValueError(f"asset package is not admitted for glass_v1 handoff: {source}")
        if audit.get("status") != "pass":
            raise ValueError(f"asset package visual material-only audit did not pass: {source}")
        ...
        records.append(
            {
                ...
                "visual_material_only_audit": (
                    f"packages/{source.name}/evidence/visual_material_only_audit.json"
                ),
            }
        )
    payload = {
        "schema_version": "scenario-forge-asset-handoff/v0.1",
        "archive_id": archive_id,
        "asset_count": len(records),
        "assets": records,
        "task_packages_modified": False,
        "claim_boundary": (
            "Four independently admitted visual-material asset packages only; no task "
            "package upgrade, robot-policy, liquid-transfer, or benchmark-success claim."
        ),
    }
...
def _readme(records: list[dict[str, object]]) -> str:
    ...
    lines.extend(
        [
            "",
            "本包没有升级任何任务 USD。它只证明材质包准入与物理/交互 profile 未变；",
