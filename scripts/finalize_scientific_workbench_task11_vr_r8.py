#!/usr/bin/env python3
"""Finalize Task 11 r8 scene qualification without robot-success promotion."""
...
def finalize(package: Path) -> Path:
    ...
    static_dir = package / "vr/evidence/r8_static"
    ...
    if not all(
        ...
        raise RuntimeError("all three hash-bound r8 static runs must pass")
    mechanical_path = package / "vr/evidence/r8_mechanical/report.json"
    ...
    if (
        ...
        raise RuntimeError("hash-bound r8 device mechanics must pass")
    ...
    if render.get("status") != "pass" or render.get("scene_usd_sha256") != scene_sha:
        raise RuntimeError("r8 render evidence is incomplete")
    required_views = {
        "scene_overview.png",
    ...
    if not required_views.issubset(render.get("views", {})):
        raise RuntimeError("r8 render evidence is missing required views")
    if review.get("verdict") != "pass" or review.get("scene_usd_sha256") != scene_sha:
        raise RuntimeError("r8 visual review is incomplete")
    ...
    if (
        ...
        raise RuntimeError("r8 GenManip adapter smoke is incomplete")

    summary = {
        "schema_version": "scenario-forge.task11-r8-static-qualification.v1",
        "status": "pass",
        "runtime": "isaac41",
    ...
    visual_manifest_path = package / "visual_liquid_manifest.json"
    visual_manifest = json.loads(visual_manifest_path.read_text())
    visual_manifest["status"] = "runtime_inspected_pass"
    visual_manifest["scene_usd_sha256"] = scene_sha
    ...
    manifest["runtime_qualification"] = {
        "scene_usd_sha256": scene_sha,
        "static": "vr/evidence/r8_static/report.json",
        "device_mechanics": "vr/evidence/r8_mechanical/report.json",
