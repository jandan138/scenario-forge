#!/usr/bin/env python3
"""Finalize the Task 12 dual-glassware oven-unload VR handoff."""
...
def finalize(root: Path) -> Path:
    ...
    root = root.resolve()
    smoke = root / "evidence/runtime/static_device_smoke.json"
    ...
    for path in (smoke, render, review):
        if json.loads(path.read_text()).get("status") != "pass":
            raise ValueError(f"Task12 evidence did not pass: {path}")
    ...
    if closure["status"] != "pass":
        raise ValueError("Task12 dependency closure blocked")

    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["status"] = "isaac41_runtime_complete"
    manifest["runtime_evidence"] = {
        "static_device_smoke": "evidence/runtime/static_device_smoke.json",
        "static_device_smoke_sha256": _sha(smoke),
    ...
    shutil.make_archive(
        str(archive.with_suffix("")), "zip", root_dir=output, base_dir=root.name
    )
