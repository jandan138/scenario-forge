...
OBJECT_NAMES = ("obj_beaker", "obj_stir_bar", *tuple(OBJECT_SOURCES)[1:])
...
def build(output: Path, base: Path, stir_bar: Path) -> Path:
    ...
    stir.AddTranslateOp().Set(Gf.Vec3d(*STIR_BAR_XYZ))
    ...
    materialize_vr_object_subtrees(
        ...
        prunable_dependency_roots=[stir_dependency],
    )
    ...
    task = {
        ...
        "layout_randomization": {
            "table": "table",
            "objects": [_local_group(name) for name in OBJECT_NAMES],
        },
        "robot_cfg": {
    ...
    scenario = {
        ...
        "instruction": (
            "辅助臂固定空烧杯；操作臂拿起磁力搅拌子，对准烧杯口并放入烧杯。"
        ),
        "steps": [
        ...
        "context_objects": [
            ...
            if name not in {"obj_beaker", "obj_stir_bar"}
        ],
    }
    ...
    manifest = {
        ...
        "package_id": "scientific_workbench_insert_stir_bar_into_beaker_vr_r1",
        "scenario_id": TASK_ID,
...
def main() -> int:
    ...
    print(build(args.out, args.base, args.stir_bar))
    return 0
