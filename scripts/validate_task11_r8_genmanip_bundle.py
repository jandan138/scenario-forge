#!/usr/bin/env python3
"""Run a package-local composition/config smoke for the Task 11 r8 adapter."""
...
def validate(bundle: Path) -> Path:
    ...
    task_files = sorted((bundle / "tasks/scenario_forge/task11_r8").rglob("*.json"))
    wrapper_text = scene_path.read_text()
    checks = {
        "stage_composes": not missing,
        "default_prim_world": str(stage.GetDefaultPrim().GetPath()) == "/World",
        "source_copy_hash_bound": (
            sha256(source_copy.read_bytes()).hexdigest()
        ...
        "config_routes_r8": "task11_r8/scene" in evaluation["usd_name"],
        "episode_metadata_present": len(task_files) == 1,
    }
    passed = all(checks.values())
    report = {
        "schema_version": "scenario-forge.task11-r8-genmanip-smoke.v1",
    ...
    if not passed:
        raise RuntimeError(f"Task 11 r8 adapter smoke blocked: {report_path}")
    return report_path
