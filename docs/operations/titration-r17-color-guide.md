# r1.7 滴定液体颜色教学：一块液体、一套材质、连续变色

适用任务包：`scientific_workbench_traditional_acid_base_titration_vr_r1_7`，Isaac Sim 4.5。
本文讲锥形瓶内的假液体；滴定管内部液体沿用原设置。

## 1. 现在只有一套液体几何与材质

r1.7 用一个 Mesh、一套 Material 和一个 Shader 表示锥形瓶内的液体。
...
if not shader:
    raise RuntimeError("没有找到液体 Shader，请确认是 r1.7 原始场景。")
...
r1.7 的接收瓶更新函数直接忽略这个返回值，只写 `inputs:glass_color`。
...
单材质结构延续 r1.6；相对 r1.5，它使用一套液体和材质，避免按阶段切换。r1.7 另修正滴嘴并更新初始位姿，见第9节。
...
单套材质集成由 `scripts/generate_traditional_titration_vr_r17.py` 完成。
交付包已经嵌入所需控制代码，运行时不依赖仓库源码。

## 9. r1.7 的滴嘴与位姿修复

颜色逻辑仍沿用r1.6，不改流量或成功规则。修复的是以下两个原有Prim：

- `/World/obj_titration_station/Instance/Burette/body_link/Visual/delivery_tip`：
  由Cone改为Mesh，路径和父子关系不变。上端接出液管，下端渐细、有开口。
- 同一Visual下的 `liquid_precharge`：保留Mesh类型，按新内腔重算下段。

原Cone尖端沿+Z指向上方，而且自定义 `asset:tip_radius2_m` 不会让标准Cone变成中空管。
新滴嘴在body_link局部z=-0.23至-0.28之间，消除原10 mm重叠，长度为50 mm。
上/下外半径为2/0.8 mm，内半径为1.55/0.4 mm；出口位置不变。
内部假液体距内壁约0.05 mm，底端缩在出口内约0.2 mm，不产生管外液滴。
Mesh几何由points和面定义，旧Cone的height/radius不再是这个节点的几何参数。

滴定站、接收瓶、搅拌器和两只旁边容器的位置从提供的ZIP精确读取。
源ZIP、场景哈希和五个位姿保存在 `evidence/initial_pose_reference.json`；
不会导入参考包的背景或物理配置。现有单材质锥形瓶液体及其读取接口不变。
